...
public interface OrdersRepository extends JpaRepository<Orders, String>{
    ...
    @Query(value="select o.id, p.price, p.name as productname, cat.name as categoryname, c.name as customername," +
            " c.email, c.phone, c.address, o.date, o.orderstatus from orders as o" +
            " left join product as p on p.id = o.productid" +
            " left join categories as cat on cat.id = p.categoryid" +
            " left join customer as c on c.id = o.customerid  where o.customerid=?1", nativeQuery = true)
